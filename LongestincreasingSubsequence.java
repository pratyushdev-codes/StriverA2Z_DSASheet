...
public class LongestincreasingSubsequence {
    ...
    public static void main(String[] args) {
    ...
    }public int calculateLength(int [] nums ,int prev_idx, int idx , int [][] dp ){
        if(idx==nums.length){
                return 0;
        }
        ...
        int len = calculateLength(nums , prev_idx, idx+1 , dp);


        if(prev_idx< nums[idx] || prev_idx==-1){
            len =Math.max(len ,  1+ calculateLength(nums, idx, idx+1, dp));
        }
